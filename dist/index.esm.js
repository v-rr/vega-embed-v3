...
import { quantile, ascending, variance, median, extent, range, mean, min, max, bisector, bisectRight, bisect, sum, bisectLeft, permute, tickStep } from 'd3-array';
...
import { __awaiter, __assign, __rest, __extends } from 'tslib';
...
function clampRange(range$$1, min$$1, max$$1) {
  ...
  return span >= (max$$1 - min$$1)
    ? [min$$1, max$$1]
    : [
        (lo = Math.min(Math.max(lo, min$$1), max$$1 - span)),
        lo + span
      ];
}
...
function bin(_$$1) {
  var maxb = _$$1.maxbins || 20,
      ...
      min$$1  = _$$1.extent[0],
      max$$1  = _$$1.extent[1],
      span = (max$$1 - min$$1) || Math.abs(min$$1) || 1,
  ...
  return {
    start: min$$1,
    stop:  max$$1 === min$$1 ? min$$1 + step : max$$1,
    step:  step
  };
}
...
function sampleCurve(f, extent$$1, minSteps, maxSteps) {
...
function point(event$$1, el) {
  var rect = el.getBoundingClientRect();
  return [
    event$$1.clientX - rect.left - (el.clientLeft || 0),
    event$$1.clientY - rect.top - (el.clientTop || 0)
  ];
}
...
function defaultTooltip(handler, event$$1, item, value) {
...
prototype$F.handleHref = function(event$$1, item, href) {
  this._loader
    .sanitize(href, {context:'href'})
    .then(function(opt) {
      var e = new MouseEvent(event$$1.type, event$$1),
          a = domCreate(null, 'a');
...
prototype$F.handleTooltip = function(event$$1, item, show) {
  if (item && item.tooltip != null) {
    item = resolveItem(item, event$$1, this.canvas(), this._origin);
    var value = (show && item && item.tooltip) || null;
    this._tooltip.call(this._obj, this, event$$1, item, value);
  }
};
...
function scaleFraction(scale, min$$1, max$$1) {
  ...
  return !delta || !isFinite(delta) ? constant(0)
    : scale.type === 'linear' || scale.type === 'sequential'
      ...
      : scale.copy().domain([min$$1, max$$1]).range([0, 1]).interpolate(lerp$1);
}
function lerp$1(a, b) {
  var span$$1 = b - a;
...
function padDomain(type, domain, range$$1, pad$$1, exponent) {
  var span$$1 = Math.abs(peek(range$$1) - range$$1[0]),
...
function cursor(view) {
  ...
  view.on(view.events('view', 'mousemove'), cursor,
    function(_$$1, event$$1) {
      var value = cursor.value,
          user = value ? (isString(value) ? value : value.user) : Default,
          item = event$$1.item && event$$1.item.cursor || null;
...
function eventExtend(view, event$$1, item) {
  var el = view._renderer.scene(),
      p, e, translate;
  if (el) {
    translate = offset$1(view);
    e = event$$1.changedTouches ? event$$1.changedTouches[0] : event$$1;
    p = point(e, el);
    p[0] -= translate[0];
    p[1] -= translate[1];
  }
  event$$1.dataflow = view;
  event$$1.vega = extension(view, item, p);
  event$$1.item = item;
  return event$$1;
}
...
function itemFilter(event$$1) {
  return event$$1.item;
}
function markTarget(event$$1) {
  var source = event$$1.item.mark.source;
  return source.source || source;
}
function invoke(name) {
  return function(_$$1, event$$1) {
    return event$$1.vega.view()
      .changeset()
      .encode(event$$1.item, name);
  };
}
...
function select$1(bind, el, param, value) {
...
function clampRange$1(range$$1, min$$1, max$$1) {
  var lo = range$$1[0],
      hi = range$$1[1],
      span;
  if (hi < lo) {
...
function pinchDistance(event$$1) {
  var t = event$$1.touches,
      dx = t[0].clientX - t[1].clientX,
      dy = t[0].clientY - t[1].clientY;
  return Math.sqrt(dx * dx + dy * dy);
}
function pinchAngle(event$$1) {
  var t = event$$1.touches;
  return Math.atan2(
...
function flush$1(range$$1, value, threshold, left, right, center) {
...
function inrange$1(value, range$$1, left, right) {
  var r0 = range$$1[0], r1 = range$$1[range$$1.length-1], t;
  if (r0 > r1) {
    t = r0;
    r0 = r1;
...
function defaultTooltip$1(handler, event$$1, item, value) {
...
function accessPathWithDatum(path$$1, datum) {
    if (datum === void 0) { datum = 'datum'; }
    var pieces = splitAccessPath(path$$1);
    var prefixes = [];
...
function flatAccessWithDatum(path$$1, datum) {
    if (datum === void 0) { datum = 'datum'; }
    return datum + "[" + $(splitAccessPath(path$$1).join('.')) + "]";
}
function replacePathInField(path$$1) {
    return "" + splitAccessPath(path$$1).map(function (p) { return p.replace('.', '\\.'); }).join('\\.');
}
function removePathFromField(path$$1) {
    return "" + splitAccessPath(path$$1).join('.');
}
function accessPathDepth(path$$1) {
    if (!path$$1) {
        return 0;
    }
    return splitAccessPath(path$$1).length;
}
    ...
    return keys$1(encoding).reduce(function (normalizedEncoding, channel) {
        ...
        if (channel === 'detail' ||
            ...
            if (fieldDef && contains([Type.LATITUDE, Type.LONGITUDE], fieldDef.type)) {
                var _b = channel, _$$1 = normalizedEncoding[_b], newEncoding = __rest(normalizedEncoding, [typeof _b === "symbol" ? _b : _b + ""]);
...
function normalizeFacetedUnit(spec, config) {
    ...
    var mark = spec.mark, width = spec.width, projection = spec.projection, height = spec.height, selection = spec.selection, _$$1 = spec.encoding, outerSpec = __rest(spec, ["mark", "width", "projection", "height", "selection", "encoding"]);
...
function normalizePathOverlay(spec, config) {
    ...
    if (lineOverlay) {
        layer.push(__assign({}, (projection ? { projection: projection } : {}), { mark: __assign({ type: 'line' }, pick$2(markDef, ['clip', 'interpolate']), lineOverlay), encoding: overlayEncoding }));
    }
    if (pointOverlay) {
        layer.push(__assign({}, (projection ? { projection: projection } : {}), { mark: __assign({ type: 'point', opacity: 1, filled: true }, pick$2(markDef, ['clip']), pointOverlay), encoding: overlayEncoding }));
    }
...
function isVgRangeStep(range$$1) {
    return !!range$$1['step'];
}
...
function baseEncodeEntry(model, ignore) {
    return __assign({}, markDefProperties(model.markDef, ignore), color$2(model), nonPosition('opacity', model), tooltip$1(model), text$2(model, 'href'));
}
...
function pointPosition(channel, model, defaultRef, vgChannel) {
    ...
    var valueRef = !channelDef && (encoding.latitude || encoding.longitude) ?
        { field: model.getName(channel) } : __assign({}, position$1(channel, encoding[channel], scaleName, scale, stack, getDefaultRef(defaultRef, channel, scaleName, scale, mark)), (offset ? { offset: offset } : {}));
    return _a = {},
...
function timeFormatExpression(field$$1, timeUnit, format$$1, shortTimeLabels, timeFormatConfig, isUTCScale, alwaysReturn) {
    if (alwaysReturn === void 0) { alwaysReturn = false; }
    if (!timeUnit || format$$1) {
        format$$1 = format$$1 || timeFormatConfig;
...
function getHeaderGroup(model, channel, headerType, layoutHeader, headerCmpt) {
    ...
    if (headerCmpt) {
        ...
        if (facetFieldDef && headerCmpt.labels) {
            ...
            title$$1 = __assign({ text: formatSignalRef(facetFieldDef, format$$1, 'parent', model.config), offset: 10, orient: channel === 'row' ? 'left' : 'top', style: 'guide-label' }, getHeaderProperties(config, facetFieldDef, HEADER_LABEL_PROPERTIES, HEADER_LABEL_PROPERTIES_MAP), (keys$1(update).length > 0 ? { encode: { update: update } } : {}));
        }
...
function sizeSignals(model, sizeType) {
    ...
    if (size === 'range-step') {
        ...
        if (scaleComponent) {
            ...
            var range$$1 = scaleComponent.get('range');
            if (hasDiscreteDomain(type) && isVgRangeStep(range$$1)) {
                var scaleName = model.scaleName(channel);
                if (isFacetModel(model.parent)) {
                    var parentResolve = model.parent.component.resolve;
                    if (parentResolve.scale[channel] === 'independent') {
                        return [stepSignal(scaleName, range$$1)];
                    }
                }
                return [
                    stepSignal(scaleName, range$$1),
                    {
                        name: name,
...
function stepSignal(scaleName, range$$1) {
    return {
...
function symbols$1(fieldDef, symbolsSpec, model, channel, type) {
    ...
    var out = __assign({}, applyMarkConfig({}, model, FILL_STROKE_CONFIG), color$2(model));
    switch (model.mark) {
        case BAR:
        case TICK:
        case TEXT$1:
...
function assembleProjectionForModel(model) {
    ...
    var name = projection.name, rest = __rest(projection, ["name"]);
    var size = {
...
var ProjectionComponent =               (function (_super) {
    ...
    function ProjectionComponent(name, specifiedProjection, size, data) {
        var _this = _super.call(this, __assign({}, specifiedProjection),
        { name: name }
        ) || this;
    ...
    FacetNode.prototype.getChildIndependentFieldsWithStep = function () {
        ...
        for (var _i = 0, _a = ['x', 'y']; _i < _a.length; _i++) {
            ...
            if (childScaleComponent && !childScaleComponent.merged) {
                ...
                var range$$1 = childScaleComponent.get('range');
                if (hasDiscreteDomain(type) && isVgRangeStep(range$$1)) {
...
var SourceNode =               (function (_super) {
    ...
    function SourceNode(data) {
        ...
        if (data.format) {
            var _a = data.format, _b = _a.parse, format$$1 = __rest(_a, ["parse"]);
            _this._data.format = format$$1;
        }
        return _this;
    }
...
function assembleScalesForModel(model) {
    return keys$1(model.component.scales).reduce(function (scales, channel) {
        ...
        var domainRaw = scale.domainRaw, range$$1 = scale.range;
        ...
        range$$1 = assembleScaleRange(range$$1, name, model, channel);
        if (domainRaw && isRawSelectionDomain(domainRaw)) {
            domainRaw = selectionScaleDomain(model, domainRaw);
        }
        scales.push(__assign({ name: name,
            type: type, domain: assembleDomain(model, channel) }, (domainRaw ? { domainRaw: domainRaw } : {}), { range: range$$1 }, otherScaleProps));
        return scales;
    }, []);
}
...
function onDelta(model, selCmpt, channel, size, signals) {
    ...
    var extent$$1 = anchor + ".extent_" + channel;
    var offset = "" + sign + delta + "." + channel + " / " + (hasScales ? "" + sizeSg : "span(" + extent$$1 + ")");
    var panFn = !hasScales ? 'panLinear' :
        scaleType === 'log' ? 'panLog' :
            scaleType === 'pow' ? 'panPow' : 'panLinear';
    var update = panFn + "(" + extent$$1 + ", " + offset +
...
function parseUnitSelection(model, selDefs) {
    ...
    var _loop_1 = function (name_1) {
        ...
        var selCmpt = selCmpts[name_1] = __assign({}, selDef, { name: name_1, events: isString(selDef.on) ? selector(selDef.on, 'scope') : selDef.on });
...
function channelSignalName(selCmpt, channel, range$$1) {
    var sgNames = selCmpt._signalNames || (selCmpt._signalNames = {});
    if (sgNames[channel] && sgNames[channel][range$$1]) {
        return sgNames[channel][range$$1];
    }
    sgNames[channel] = sgNames[channel] || {};
    var basename = varName(selCmpt.name + '_' + (range$$1 === 'visual' ? channel : selCmpt.fields[channel]));
    var name = basename;
    var counter = 1;
    while (sgNames[name]) {
        name = basename + "_" + counter++;
    }
    return (sgNames[name] = sgNames[channel][range$$1] = name);
}
...
function defaultUnitSize(model, sizeType) {
    ...
    if (scaleComponent) {
        var scaleType = scaleComponent.get('type');
        var range$$1 = scaleComponent.get('range');
        if (hasDiscreteDomain(scaleType) && isVgRangeStep(range$$1)) {
            return 'range-step';
        }
        else {
...
var area$3 = {
    ...
    encodeEntry: function (model) {
        return __assign({}, baseEncodeEntry(model, { size: 'ignore', orient: 'include' }), pointPosition('x', model, 'zeroOrMin'), pointPosition('y', model, 'zeroOrMin'), pointPosition2(model, 'zeroOrMin', model.markDef.orient === 'horizontal' ? 'x2' : 'y2'), defined$1(model));
    }
};
...
var trail$2 = {
    ...
    encodeEntry: function (model) {
        var width = model.width, height = model.height;
        return __assign({}, baseEncodeEntry(model, { size: 'include', orient: 'ignore' }), pointPosition('x', model, mid(width)), pointPosition('y', model, mid(height)), nonPosition('size', model), defined$1(model));
    }
};
...
var rect$1 = {
    vgMark: 'rect',
    encodeEntry: function (model) {
        return __assign({}, baseEncodeEntry(model, { size: 'ignore', orient: 'ignore' }), x$2(model), y$2(model));
    }
};
function x$2(model) {
...
var text$3 = {
    ...
    encodeEntry: function (model) {
        ...
        return __assign({}, baseEncodeEntry(model, { size: 'ignore', orient: 'ignore' }), pointPosition('x', model, mid(width)), pointPosition('y', model, mid(height)), text$2(model), nonPosition('size', model, __assign({}, (markDef.size ? { defaultValue: markDef.size } : {}), { vgChannel: 'fontSize'
...
function getTopLevelProperties(topLevelSpec, config, autosize) {
    return __assign({ autosize: keys$1(autosize).length === 1 && autosize.type ? autosize.type : autosize }, extractTopLevelProperties(config), extractTopLevelProperties(topLevelSpec));
}
...
function formatValue$2(value, valueToHtml, maxDepth) {
    ...
    if (isObject(value)) {
        let content = '';
        const _a = value, { title } = _a, rest = __rest$1(_a, ["title"]);
        if (title) {
...
function calculatePosition(event$$1, tooltipBox, offsetX, offsetY) {
    let x = event$$1.clientX + offsetX;
    if (x + tooltipBox.width > window.innerWidth) {
        x = +event$$1.clientX - offsetX - tooltipBox.width;
    }
    let y = event$$1.clientY + offsetY;
    if (y + tooltipBox.height > window.innerHeight) {
        y = +event$$1.clientY - offsetY - tooltipBox.height;
    }
    return { x, y };
}
...
class Handler$1 {
    ...
    tooltip_handler(handler, event$$1, item, value) {
        if (value == null || value === '') {
        ...
        const { x, y } = calculatePosition(event$$1, this.el.getBoundingClientRect(), this.options.offsetX, this.options.offsetY);
